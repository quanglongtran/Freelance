...
use Firebase\JWTCustom\JWTCustom;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Cookie;

class ChatRoomControllerV2 extends Controller
{
    public $is_mobile;
    ...
    {
        $token = Auth::attempt(['email'=>'long1@gmail.com', 'password' => '123456']);
        return ['data'=>$token];
    }
    ...
    public function __construct(Request $request)
    {
        ...
        if (!empty($request->token)) {
            $user_id = user_token::where('token', $request->token)->first()->user_id;
            // echo $user_id;
            if (!is_null($user_id)) {
                $user = Users::where('user_id', $user_id)->first();
                // $user = Users::where('user_id', 10004724)->first();
                session(['user' => $user]);
                // $this->is_mobile = true;
            }
        }
    ...
    /**
     * Get all specialist doctor list by Id
     * 
     * @param int $id
     */
    public function getDoctorSpecialityById($id)
    {
        $doctor_id = (array) DB::table('doctor_speciality')
        ->select('doctor_id')->where('speciality_id', $id)->get();
        
        $list_doctor_id = [];

        foreach ($doctor_id as $value) {
            $list_doctor_id[] = $value->doctor_id;
        }

        return DB::table('doctor')->whereIn('doctor_id', $list_doctor_id)->get();
    }
    ...
    public function getMessages(Request $rq)
    {
        ...
        $dataQuery = chat_room_messages::whereIn('room_id', function ($query) use ($rq) {
            $query->select('id')->from(with(new chat_rooms)->getTable())->where('name', $rq->create_room);
        })
        ->orderBy('id', 'desc')->paginate(50);

        $dataQuery->getCollection()->transform(function ($value) use ($rq) {
